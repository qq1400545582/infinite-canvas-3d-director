...
/**
 * Resolve the deployment base (service worker scope) so cached asset paths
 * are relative to the current deployment path. Served from the domain root
 * this is "/" (identical to the original behavior). When embedded under a
 * subpath (e.g. /openreel-video/) it scopes all caching to that subpath and
 * prevents the worker from caching or serving the host application's assets.
 */
function scopeBase() {
  try {
  ...
  event.waitUntil(
    caches
      ...
      .then((cache) => {
        ...
        const STATIC_ASSETS = [
          scopeBase(),
          scopeBase() + "index.html",
          scopeBase() + "manifest.json",
        ];
      ...
      fetch(request)
        ...
        .catch(() => {
          ...
          return caches.match(request).then((cachedResponse) => {
            ...
            return caches.match(scopeBase() + "index.html");
          });
        })
    );
    return;
