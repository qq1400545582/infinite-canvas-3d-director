export default {
    ...
    expertLibrary: {
        title: "专家库",
        subtitle: "克隆自 WorkBuddy 的专家中心、技能市场与连接器，可在画布系统中自由调用",
        tabs: { expert: "专家", skill: "技能", connector: "连接器" },
        category: "分类",
        search: "搜索名称、简介或标签",
        total: "共 {{count}} 个{{kind}}",
        detail: "详情",
        invoke: "调用",
        invoked: "已为你打开画布智能体并预填调用提示",
        agentNotRunning: "未检测到本地 Canvas Agent，请先按引导启动并连接后再调用",
        invokeHint: "在右侧智能体面板中发送，即可在画布中调用",
        drawerPrereq: "调用前需先启动并连接本地 Canvas Agent：终端执行 npx -y @basketikun/canvas-agent@latest，再在智能体面板连接。",
        agentSetup: {
            title: "未检测到本地 Canvas Agent",
            desc: "「调用」需要本机运行的 Canvas Agent 服务（默认 http://127.0.0.1:17371）。请先启动并连接，再回来点击调用。",
            step1Title: "1. 启动 Canvas Agent",
            step1: "在终端执行 npx -y @basketikun/canvas-agent@latest，或在 Codex 中用对应插件启动。",
            step2Title: "2. 获取连接凭证",
            step2: "启动后会打印 Local URL 与 Connect token，并写入 ~/.infinite-canvas/canvas-agent.json。",
            step3Title: "3. 在画布中连接",
            step3: "打开智能体面板 → 连接/设置页，粘贴 Connect token 并点击连接（Local URL 会自动读取）。",
            note: "连接成功后再次点击「调用」，即可正常使用上传图片、输入提示词、设置权限与发送。",
            confirm: "我知道了",
        },
        features: "功能与能力",
        tags: "标签",
        ...
        createExpert: {
            ...
            tabAi: "AI 创建",
            tabManual: "手动创建",
            aiDesc: "描述你想要的专家角色，AI 会生成完整的专家画像（名称 / 分类 / 简介 / 能力点 / 调用提示），确认后即可加入「我的专家」并作为画布节点调用。",
            aiPlaceholder: "例如：一个擅长小红书爆款文案的写作专家，熟悉标题技巧与平台调性。",
            aiFallbackRequirement: "一个通用的全能型专家",
            aiHint: "点击下方按钮后，会在右侧智能体面板中发送需求，并自动回填 AI 的回复。",
            aiCreate: "AI 创建",
            aiSent: "已把需求发给画布智能体，等待回复后点「解析并创建」即可。",
            aiOutput: "AI 输出",
            aiOutputPlaceholder: "AI 回复将自动显示在这里，你也可以手动粘贴专家画像后解析。",
            aiWatching: "监听中",
            aiCaptured: "已捕获 AI 回复",
            parseCreate: "解析并创建",
            parseToForm: "填入表单",
            parseFailed: "未能从内容中解析出专家画像，请检查格式或手动填写。",
            parseFailedHint: "需要至少包含「名称」与「简介」。可让 AI 重新生成，或切到「手动创建」直接填写。",
            fieldName: "专家名称",
            fieldNamePlaceholder: "例如：小红书文案专家",
            fieldNameRequired: "请填写专家名称",
            fieldCategory: "分类",
            fieldCategoryPlaceholder: "例如：写作创作",
            fieldDescription: "简介",
            fieldDescriptionRequired: "请填写专家简介",
            fieldFeatures: "能力点",
            fieldFeaturesExtra: "每行一条，描述该专家擅长的具体能力。",
            fieldTags: "标签",
            fieldTagsPlaceholder: "例如：文案、小红书、爆款",
            fieldTagsExtra: "用于检索，多个标签用空格或顿号分隔。",
            fieldCallPrompt: "调用提示",
            fieldCallPromptExtra: "在画布中调用该专家时，预填到智能体输入框的指令前缀。",
            manualCreate: "保存为我的专家",
            defaultCallPrompt: "请以「{{name}}」专家的身份，结合我的需求帮我完成下面的任务：",
            created: "已创建专属专家：{{name}}",
        },
        addSkill: {
            title: "添加技能",
            installedCount: "已安装 {{count}}",
            needAgentTitle: "未连接到本地 Canvas Agent",
            needAgentDesc: "导入 / 创建 / 从链接安装技能都依赖本机运行的 Canvas Agent，请先连接后再继续。",
            connectAgent: "去连接",
            tabImport: "导入技能",
            tabCreate: "创建技能",
            tabLink: "从链接安装",
            dropHint: "拖入或点击选择 SKILL.md / 技能文件夹 / .zip；支持多选与文件夹",
            autoInstall: "解析出安全技能后自动安装（跳过风险项）",
            requirements: "技能文件要求",
            requirement1: "必须包含 SKILL.md（或至少一个 .md），且 frontmatter 声明 name 与 description。",
            requirement2: "name 仅允许小写字母、数字与连字符（如 my-skill）；description 不超过 1024 字且不含尖括号。",
            chooseZip: "选择 .zip 文件",
            chooseFolder: "选择技能文件夹",
            parsedSummary: "已解析 {{total}} 个技能（安全 {{safe}} · 风险 {{risky}}）",
            installSelected: "安装所选技能",
            risky: "风险",
            parseFailures: "{{count}} 个文件未能解析",
            noParseResult: "未从所选文件 / 文件夹中解析出可安装的技能。",
            autoInstalling: "正在自动安装 {{count}} 个安全技能…",
            installed: "已安装 {{count}} 个技能到本机 Canvas Agent",
            installPartial: "部分技能安装失败：{{failed}}",
            installFailed: "安装失败",
            originManual: "手动创建",
            aiCreate: "AI 创建技能",
            aiPromptPrefix: "把需求发给 skill-creator，让它生成技能：",
            aiPromptSuffix: "（发送后回到这里，把生成的 SKILL.md 内容粘贴到下方手动表单，或直接在「导入」里上传。）",
            aiPlaceholder: "描述你想让这个技能做什么",
            aiHint: "AI 会生成完整技能文件，复制回「导入」或「创建」即可完成安装。",
            aiButton: "让 AI 创建",
            aiExample: "例如：一个把中文技术文章改写成英文 README 的技能",
            manualCreate: "手动创建技能",
            manualButton: "创建并安装",
            fieldName: "技能名称（name）",
            fieldNameRequired: "请填写技能名称",
            fieldNamePattern: "名称仅允许小写字母、数字与连字符，且不能以连字符开头或结尾",
            fieldDisplayName: "展示名称",
            fieldDisplayNamePlaceholder: "在智能体技能列表中显示的名字（可选）",
            fieldDescription: "描述（description）",
            fieldDescriptionRequired: "请填写技能描述",
            fieldDescriptionAngle: "描述不能包含尖括号 < 或 >",
            fieldInstructions: "技能正文（instructions）",
            fieldInstructionsRequired: "请填写技能正文",
            linkHint: "粘贴 GitHub / Gitee 仓库或单个 SKILL.md / .zip 直链，自动解析并安装其中的技能。",
            linkPlaceholder: "https://github.com/owner/repo 或 SKILL.md 直链",
            linkResolve: "解析并安装",
            linkInstallHint: "支持仓库根目录、子路径（如 /skills/my-skill）与指定分支（?ref=...）。",
            remoteFound: "已从链接解析出 {{count}} 个技能",
            errors: {
                nameInvalid: "名称不符合规范（仅小写字母、数字、连字符）",
                descriptionMissing: "缺少 description",
                descriptionTooLong: "description 超过 1024 字",
                descriptionHasAngleBrackets: "description 包含尖括号",
                instructionsMissing: "缺少技能正文（instructions）",
                instructionsTooLarge: "技能正文超过 256KB",
                frontmatterMissing: "缺少合法的 frontmatter（name / description）",
                noMarkdownFile: "未找到 SKILL.md 或 .md 文件",
            },
            remoteErrors: {
                invalidUrl: "链接格式无效，请检查后重试",
                unsupported: "暂不支持该来源，请使用 GitHub / Gitee 或直链",
                network: "网络请求失败，请检查链接或本机网络",
                notFound: "未找到该仓库或文件（404）",
                noSkills: "链接中未解析出任何技能（SKILL.md）",
                tooMany: "技能数量过多，请缩小范围或指定子路径",
            },
        },
        myConnectors: {
            title: "我的连接器",
        ...
        addConnector: {
            title: "连接器管理",
            subtitle: "配置 MCP 服务与自定义连接器，为画布智能体扩展更多工具能力",
            note: "配置保存在本地浏览器，画布智能体在「调用」时按需接入，不会在页面内建立常驻连接。请只接入你信任的服务。",
            search: "搜索连接器",
            configure: "配置连接器",
            importText: "文本导入",
            importLink: "从链接安装",
            ...
            noMatch: "没有匹配的连接器",
            empty: "暂无自定义连接器",
            emptyHint: "点击「配置连接器」添加 MCP 服务或自定义连接器",
            createTitle: "配置连接器",
            editTitle: "编辑连接器",
            back: "返回列表",
            fieldName: "服务名称",
            fieldNamePlaceholder: "例如 github-mcp",
            fieldNameRequired: "请填写服务名称",
            fieldTransport: "传输方式",
            transportStdio: "本地命令",
            transportSse: "SSE",
            transportHttp: "HTTP",
            fieldCommand: "启动命令",
            fieldCommandPlaceholder: "例如 npx / uvx / node / python",
            fieldCommandRequired: "请填写启动命令",
            fieldArgs: "启动参数",
            fieldArgsPlaceholder: "每行一个参数，例如：\n-y\n@modelcontextprotocol/server-github",
            fieldUrl: "服务地址",
            fieldUrlPlaceholder: "https://example.com/mcp",
            fieldUrlRequired: "请填写合法的 http(s) 服务地址",
            fieldEnv: "环境变量",
            fieldHeaders: "请求头",
            pairKey: "名称",
            pairValue: "值",
            addPair: "添加一项",
            fieldDescription: "用途说明",
            fieldDescriptionPlaceholder: "说明这个连接器能为智能体提供什么能力（可选）",
            enabled: "启用该连接器",
            disabledTag: "已停用",
            save: "保存",
            update: "保存修改",
            savedCount: "已保存 {{count}} 个连接器",
            updated: "连接器已更新",
            removed: "已删除连接器：{{name}}",
            edit: "编辑",
            remove: "删除",
            removeConfirm: "删除这个连接器？",
            enabledToast: "已启用：{{name}}",
            disabledToast: "已停用：{{name}}",
            importHint: "粘贴 MCP 配置即可批量导入：支持 {\"mcpServers\": {…}}、{\"servers\": {…}}、数组或单个服务对象。",
            importPlaceholder: '{\n  "mcpServers": {\n    "github": {\n      "command": "npx",\n      "args": ["-y", "@modelcontextprotocol/server-github"]\n    }\n  }\n}',
            parse: "解析",
            parsedCount: "已解析 {{count}} 个连接器",
            saveAll: "全部保存",
            linkHint: "支持 GitHub / Gitee 仓库、仓库内配置文件直链（.mcp.json / mcp.json / connectors.json），或 README 中的配置代码块。",
            linkPlaceholder: "https://github.com/owner/repo 或配置文件直链",
            resolve: "解析并预览",
            errors: {
                invalidJson: "JSON 格式不正确，请检查引号与逗号",
                noServers: "未找到可识别的连接器配置",
                nameMissing: "配置缺少服务名称",
                endpointMissing: "配置缺少启动命令或服务地址",
                emptyText: "请先粘贴配置内容",
                tooMany: "一次最多导入 50 个连接器",
                invalidUrl: "链接格式无效，请检查后重试",
                unsupported: "暂不支持该来源，请使用 GitHub / Gitee 或配置文件直链",
                network: "网络请求失败，请检查链接或本机网络",
                notFound: "仓库中未找到连接器配置",
            },
        },
        pluginTabs: {
            openLibrary: "打开专家库",
            invoke: "调用",
            placed: "已作为节点放置到画布",
            noCanvas: "请先打开一个画布，再选择「作为节点调用」",
            count: "共 {{count}} 个{{kind}}",
            empty: "暂无{{kind}}",
            kindHint: {
                expert: "专家 = 可被调用的角色化能力，自带提示词与能力清单",
                skill: "技能 = 可安装到本机 Agent 的 SKILL.md 能力包",
                connector: "连接器 = MCP 风格的外部服务接入配置（stdio / SSE / HTTP）",
            },
        },
    },
    assets: {
    ...
    canvas: {
        ...
        toolbar: {
            ...
            background: "背景图片 / 视频", uploadBackground: "上传背景", replaceBackground: "更换背景", clearBackground: "清除背景", backgroundHint: "图片或视频格式不限，文件保存在本地媒体库中", backgroundOpacity: "背景透明度", fontOpacity: "字体透明度", nodeOpacity: "节点透明度", resetOpacity: "点击复位为 100%", backgroundFailed: "背景上传失败",
        },
        project: {
    ...
    topNav: {
        ...
        downloadDesktopFallback: "暂未发布可直链的安装包，已打开桌面端发布页",
    },
    home: {
        promptError: "获取提示词失败",
    ...
    version: {
        ...
        online: {
            ...
            apply: "立即更新",
            rollback: "回滚",
            available: "可更新到 {{version}}",
            upToDate: "已是最新版本",
            protectedHint: "二次开发内容将自动保留（保护 {{count}} 项）",
            baseVersion: "二开基线 {{version}}",
            impactOverwrite: "覆盖上游文件",
            impactAdd: "新增上游文件",
            impactMerge: "三方合并",
            impactKeepLocal: "保留本地",
            impactSkipped: "二开保护跳过",
            verifyPassed: "二开完整性校验全部通过（{{total}} 项）",
            verifyFailed: "二开完整性校验未通过 {{count}} 项，已自动回滚",
            conflicts: "{{count}} 处上游改动与二开重叠，已保留本地版本",
            rolledBack: "已回滚到更新前的状态。",
            restartHint: "更新完成：版本号在构建期注入，请重启开发服务（或重新构建）后生效。",
            devOnly: "在线更新需要本地开发服务：请在项目根目录执行下方命令（支持 --check 预演、--rollback 回滚）。",
            desktopNote: "桌面端请使用下方的三层更新（插件 / 前端 / 桌面壳），无需重新下载整个应用。",
            failed: "操作失败，请查看日志",
        },
        layer: {
            title: "三层更新（桌面端）",
            desktopNote: "你正在使用桌面端。三层更新覆盖插件 / 前端 / 桌面壳，无需重新下载整个应用。",
            plugins: "插件",
            frontend: "前端 bundle",
            shell: "桌面壳",
            current: "当前：{{version}}",
            remote: "远程：{{version}}",
            noRemote: "远程：未配置",
            changed: "{{count}} 个文件待更新",
            upToDate: "已是最新",
            notConfigured: "未配置远程 manifest（CANVAS_UPDATE_MANIFEST_URL）",
            check: "检查前端更新",
            apply: "应用前端更新",
            checking: "检查中...",
            updating: "更新中...",
            applied: "前端更新已下载，重启桌面端后生效",
            shellAvailable: "桌面壳有可用更新（{{version}}）",
            shellDownloaded: "桌面壳更新已下载，重启后生效",
            installShell: "重启并安装",
            noShellUpdate: "桌面壳已是最新",
            source: "更新源",
            sourceFromEnv: "由环境变量指定",
            sourcePlaceholder: "https://你的站点/build-manifest.json",
            saveSource: "保存",
            sourceSaved: "已保存",
            sourceHint: "填入你部署站点的 build-manifest.json 地址（如 Cloudflare Pages / Nginx），保存后即可检查并增量更新前端。",
            invalidUrl: "请输入以 http(s):// 开头的地址",
            needSource: "请先填写并保存更新源",
        },
    },
    config: {
        title: "配置与用户偏好",
        ...
        channelEditor: {
            ...
            sync: "同步到 Codex",
            syncHint: "把本渠道的接口地址、API Key 与文本模型写入本机 Codex 配置（自动备份原文件），Agent 对话即可复用同一供应商。改完需重启本地 Agent。",
            synced: "已同步到 Codex（模型：{{model}}），重启本地 Agent 后生效。",
            syncMissing: "请先填写接口地址和 API Key。",
            syncProtocolUnsupported: "仅 OpenAI 协议的渠道可同步到 Codex。",
            syncNoModel: "该渠道还没有模型，请先选择模型并把其中一个指定为「文本」。",
            syncUnavailable: "当前环境不支持同步（需通过本地开发服务访问）。",
            syncFailed: "同步到 Codex 失败。",
            capabilities: {
                image: "生图",
                video: "视频",
                text: "文本",
    ...
    agent: {
        ...
        connect: { pluginTitle: "方式一：在 Codex 中使用插件", pluginText: "在 Codex app 安装 Infinite Canvas 插件后，通过插件启动画布，插件会自动启动本地 Agent 并带上连接信息。", directTitle: "方式二：画布内一键启动", directText: "点击下方按钮即可在画布内直接启动本地 Agent，无需在终端输入任何命令；启动后会自动连接。", commandCopied: "命令已复制", pluginReminder: "Codex 插件提醒", pluginReminderText: "只有安装 Codex 插件或手动添加 MCP 后，工具列表才会进入 Codex 上下文并增加 token 消耗；仅运行 npx -y @basketikun/canvas-agent@latest 启动本地 Agent 不会安装 MCP。", removePlugin: "移除插件", removeMcp: "移除手动 MCP", copyCommand: "复制命令", title: "连接本地 Agent", description: "按使用场景选择一种连接方式。", webConnection: "网页连接", autoDiscover: "默认自动读取 Local URL 和 Connect token，失败时再手动填写。", conversationState: "当前对话：{{state}}", disconnect: "断开", connect: "连接", localAddress: "本地地址", urlPlaceholder: "例如 http://127.0.0.1:17371", token: "连接 Token", tokenPlaceholder: "自动发现，或手动填入 Connect token" , startBackend: "一键启动后端", startingBackend: "正在启动本地 Agent…", startBackendFailed: "启动失败，请手动启动后重试", startBackendLocalOnly: "一键启动只在「本机地址」下可用：请改用 localhost（或 127.0.0.1）在本机打开画布，或手动启动本地 Agent 后再连接。", startBackendUnavailable: "当前环境没有「一键启动后端」接口：网页版 / 静态部署的页面无法拉起你本机上的进程。请先在本机启动 Canvas Agent，再回到这里点「连接」。", startBackendHint: "无需在终端输入命令，点击下方按钮即可在画布内直接启动本地 Agent（默认 127.0.0.1:17371）。", interopTitle: "在 Codex / Trae / DSH 中以插件形式调用", interopText: "将本机 Agent 作为 MCP 服务注册到下方工具，即可在工具内直接调用画布能力。DeepSeek Harness（DSH）桌面端已内置并自动启动本 Agent。", interopConfig: "MCP 配置（任意支持 MCP 的工具通用）", interopCopy: "复制配置", interopNote: "将上述 JSON 添加到 Codex / Trae / Claude Desktop / VS Code 等支持 MCP 的工具配置中即可互通。", lnaTitle: "需要授权「本地网络访问」", lnaText: "你正在用公网网页连接本机 Agent（127.0.0.1）。浏览器会要求授权「私有网络访问」：在弹出的权限框里点「允许」后重试连接；授权后 HTTPS 页面即可调用本机 Agent，mixed content 也会被豁免。", lnaChromium: "提示：建议使用 Chrome / Edge 等 Chromium 内核浏览器。Firefox 暂未实现该授权机制，可能无法直连本机 Agent。", lnaFirefox: "提示：当前浏览器可能不支持「本地网络访问」授权机制。建议改用 Chrome / Edge 等 Chromium 内核浏览器连接本机 Agent。"},
    ...
    onboarding: {
        title: "已为你预装 4 个画布插件",
        desc: "这些自研插件已默认启用，无需任何配置即可在「新建节点」里使用。",
        enabledNote: "它们已默认启用；如需关闭或查看，到「插件」面板管理。",
        manager: "打开插件面板",
        gotIt: "知道了",
        plugin: {
            clipshot: "分镜工作台 · 把剧本拆成可自定义格数的分镜并逐格出图",
            directorDesk: "白模预演台 · 导入模型、布置机位灯光、导出预演视频",
            jlmlh: "3D 导演台 · 上游背景同步、机位时间线、截图导出画布",
            openreel: "OpenReel 视频编辑器 · 时间线剪辑、转场、滤镜、字幕、ffmpeg 导出",
        },
    },
    locale: {
        zhCN: "简体中文",
