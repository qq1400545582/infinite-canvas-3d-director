...
class ServiceWorkerManager {
  ...
  async register(): Promise<ServiceWorkerRegistration | null> {
    ...
    try {
      // Compute the deployment base so the service worker is scoped to the
      // current path. At the domain root this is identical to the original
      // behavior ("/sw.js" + scope "/"). When embedded as a canvas plugin
      // under a subpath (e.g. /openreel-video/), the worker is scoped to that
      // subpath so it can never hijack or cache the host application.
      const base =
