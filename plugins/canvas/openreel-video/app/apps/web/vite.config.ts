...
// 子路径托管开关（无限画布节点插件：产物挂在 /openreel-video/ 下）：
// 与 desktop 一样需要相对资源路径，但【不】触发 strip-ffmpeg / prune-fonts，
// 因此 ffmpeg 多线程 core、字体等全部能力保持不变。
const isSubpath = process.env.OPENREEL_BASE_RELATIVE === "1";

function desktopHtmlPlugin() {
  return {
    name: "openreel-desktop-html",
    transformIndexHtml(html: string) {
      if (!isDesktop && !isSubpath) return html;
      let out = html
