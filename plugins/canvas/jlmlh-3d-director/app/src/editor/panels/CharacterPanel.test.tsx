...
import { hashIdToPhase } from "../runtime/mannequin/motion";
...
it("动作开启时姿势滑杆渲染动作默认数据（偏移 0 = 动作当前值）", async () => {
  const user = userEvent.setup();
  // 把播放头对到 θL=π/2（左髋前摆峰值 25°）
  const phase = hashIdToPhase("char_default_a");
  const t = (Math.PI / 2 - phase) / (Math.PI * 2);
  usePlaybackStore.setState({ playhead: t });
  ...
  expect(screen.getByLabelText("左髋 · 前抬")).toHaveValue(25);
  expect(screen.getByLabelText("右髋 · 前抬")).toHaveValue(-12);
  // 左膝着地伸直
  expect(screen.getByLabelText("左膝 · 弯曲")).toHaveValue(0);
});

it("动作开启时拖动滑杆写入的是偏移而非绝对值", async () => {
  const user = userEvent.setup();
  const phase = hashIdToPhase("char_default_a");
  const t = (Math.PI / 2 - phase) / (Math.PI * 2);
  usePlaybackStore.setState({ playhead: t });
  ...
  await user.click(screen.getByRole("button", { name: "姿势" }));
  await user.clear(screen.getByLabelText("左髋 · 前抬"));
  await user.type(screen.getByLabelText("左髋 · 前抬"), "40");

  // 40 = 动作值 25 + 偏移 15（度数↔弧度往返有浮点尾差，用 toBeCloseTo）
  expect(
    useDirectorStore.getState().project.objects.find((item) => item.id === "char_default_a")?.characterRig?.controls[
      "leftHip.pitch"
    ]
  ).toBeCloseTo(15);
});
