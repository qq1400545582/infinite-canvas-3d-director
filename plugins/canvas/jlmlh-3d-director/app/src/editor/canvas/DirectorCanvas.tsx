...
import { setVideoExportContext } from "../io/videoExportBridge";
...
import { PlaybackTransformSync } from "./PlaybackTransformSync";
...
/** 把渲染三件套注册给视频导出逻辑（逐帧离线渲染用），模式仿 CanvasCaptureBridge */
function VideoExportBridge() {
  const { camera, gl, scene } = useThree();

  useEffect(() => {
    setVideoExportContext({ gl, scene, camera: camera as ThreePerspectiveCamera });
    return () => setVideoExportContext(null);
  }, [camera, gl, scene]);
