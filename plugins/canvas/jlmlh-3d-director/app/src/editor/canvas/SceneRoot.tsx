...
function ViewportTransformControls({
  ...
  const setEditingObjectId = usePlaybackStore((state) => state.setEditingObjectId);
  ...
  return (
    <TransformControls
      ...
      onMouseDown={() => {
        beginUndoBatch();
        // 拖拽期间标记该对象为“编辑中”，PlaybackTransformSync 会跳过它，避免覆盖拖拽值
        setEditingObjectId(editingId ?? null);
      }}
...
function ObjectSceneNode({
  ...
  const motionTime = usePlaybackStore((state) => state.playhead);
  ...
  const node = (
    <group
      ...
      userData={{ directorObjectId: item.id }}
      onClick={(event) => {
...
export function getViewportCameraFrustumLines(
  ...
  const halfWidth = frameDepth * Math.tan((camera.fov * Math.PI) / 360);
  const halfHeight = halfWidth / VIEWPORT_CAMERA_ASPECT;
...
function ViewportCameraRig({
  ...
  const node = (
    <group
      ...
      userData={{ [HIDE_FROM_VIEWPORT_CAPTURE_KEY]: true, directorCameraId: camera.id }}
