...
it("uses a 16:9 front viewfinder sized by the camera fov", () => {
  const metrics = getFrustumMetrics({});
  const expectedDepth = 5.2 * VIEWPORT_CAMERA_VISUAL_SCALE;
  const expectedHalfWidth = expectedDepth * Math.tan((50 * Math.PI) / 360);

  expect(metrics.maxDepth).toBeCloseTo(expectedDepth);
  expect(metrics.maxHalfWidth).toBeCloseTo(expectedHalfWidth);
  expect(metrics.maxHalfHeight).toBeCloseTo(expectedHalfWidth / (16 / 9));
...
it("scales the viewport camera viewfinder size with fov", () => {
  const narrowMetrics = getFrustumMetrics({ fov: 35 });
  const defaultMetrics = getFrustumMetrics({ fov: 50 });
  const wideMetrics = getFrustumMetrics({ fov: 75 });

  expect(narrowMetrics.maxHalfWidth).toBeLessThan(defaultMetrics.maxHalfWidth);
  expect(defaultMetrics.maxHalfWidth).toBeLessThan(wideMetrics.maxHalfWidth);
});
