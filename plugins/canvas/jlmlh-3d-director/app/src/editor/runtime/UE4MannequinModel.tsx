...
import { applyMotionOverrides } from "./mannequin/motion";
import {
...
interface UE4MannequinModelProps {
  ...
  /** 全局时间（秒）= playbackStore.playhead；驱动基础动作循环。 */
  motionTime?: number;
  /** 稳定 id，用于群众相位偏移。 */
  characterId?: string;
}
...
export function UE4MannequinModel({
  ...
  const rigControls = rigState?.controls ?? {};
  // 基础动作（走/跑）按 motionTime 计算关节角覆盖后合并进 controls（度数域）。
  // 播放/scrub 时 motionTime 每帧变化 → effect 每帧重摆姿势，即动画本体。
  const controlsWithMotion = useMemo(
    () => applyMotionOverrides(rigControls, { motion: rigState?.motion, time: motionTime ?? 0, characterId: characterId ?? "" }),
    [rigControls, rigState?.motion, motionTime, characterId]
  );
  ...
  }, [bodyType, color, controlsWithMotion, onLabelAnchorYChange, restPose, scene]);
