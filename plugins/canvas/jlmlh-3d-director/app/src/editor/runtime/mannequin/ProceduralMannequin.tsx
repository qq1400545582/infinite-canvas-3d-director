...
import { applyMotionOverrides } from "./motion";

interface ProceduralMannequinProps {
  bodyType?: CharacterBodyType;
  color?: string;
  rigState?: CharacterRigState;
  /** 全局时间（秒），通常 = playbackStore.playhead。仅在 motion.kind !== "none" 时被使用。 */
  motionTime?: number;
  /** 稳定 id，用于驱动 phase 偏移；缺省时所有个体相位一致。 */
  characterId?: string;
}
...
export function ProceduralMannequin({ bodyType, color = "#4F8EF7", rigState, motionTime, characterId }: ProceduralMannequinProps) {
  const preset = getBodyPreset(bodyType);
  const controls = applyMotionOverrides(rigState?.controls ?? {}, {
