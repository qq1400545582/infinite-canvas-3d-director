...
export interface DirectorActions {
  ...
  // 基础动作（走路 / 跑步）
  setMotion: (id: string, kind: MotionKind) => void;
  setCrowdMotion: (crowdId: string, kind: MotionKind) => void;
  setMotionSpeed: (id: string, speed: number) => void;
  setCrowdMotionSpeed: (crowdId: string, speed: number) => void;
  setMotionAmplitude: (id: string, amplitude: number) => void;
  setCrowdMotionAmplitude: (crowdId: string, amplitude: number) => void;
  setMotionEnabled: (id: string, enabled: boolean) => void;
  setCrowdMotionEnabled: (crowdId: string, enabled: boolean) => void;
...
/** 把 motion 字段的 patch 应用到单个 character；若没有 rig 则保持不变。 */
function applyMotionPatchToCharacter(
  character: DirectorObject,
  patch: Partial<{ kind: MotionKind; speed: number; amplitude: number; enabled: boolean }>
): DirectorObject {
  if (!character.characterRig) return character;
  const current = character.characterRig.motion ?? {
    kind: "none" as MotionKind,
    speed: 1,
    amplitude: 1,
    enabled: true,
  };
  return {
    ...
    characterRig: {
      ...
      motion: {
        ...
        speed: typeof patch.speed === "number" ? patch.speed : current.speed,
        amplitude: typeof patch.amplitude === "number" ? patch.amplitude : current.amplitude,
...
/** 姿势与动作互斥（选择层面）：
 *  - 任何动作切换（无→walk、walk→run、run→无…）都把姿势数据整体重置为
 *    动作默认状态（站立基础态），避免上一动作/姿势的偏移残留到下一动作
 *  - 重复选择同一动作不触发重置 */
function applyMotionSelectionToCharacter(character: DirectorObject, kind: MotionKind): DirectorObject {
  if (!character.characterRig) return character;
  const current = character.characterRig.motion ?? {
    kind: "none" as MotionKind,
    speed: 1,
  ...
  const nextRig: NonNullable<DirectorObject["characterRig"]> = {
    ...character.characterRig,
    motion: { ...current, kind },
  };
  if (kind !== current.kind) {
    nextRig.posePresetId = "stand";
    nextRig.controls = {};
  }
  return { ...character, characterRig: nextRig };
}
  ...
  return {
    ...
    deleteSelectedObject: () =>
      commitMutation((state) => {
        ...
        // 同步清理被删对象的关键帧轨道：物体轨道按 objectId、相机轨道按 linkedCameraId
        const nextTracks = state.project.timeline.tracks.filter(
          (track) => !selectedObjectIdSet.has(track.objectId)
        );
        const nextCameraTracks = state.project.timeline.cameraTracks.filter(
          (track) => !linkedCameraIds.has(track.cameraId)
        );
        const timelineUnchanged =
          nextTracks.length === state.project.timeline.tracks.length &&
          nextCameraTracks.length === state.project.timeline.cameraTracks.length;
        ...
        return {
          ...
          project: {
            ...
            timeline: timelineUnchanged
              ? state.project.timeline
              : {
                  ...state.project.timeline,
                  tracks: nextTracks,
                  cameraTracks: nextCameraTracks,
                },
          },
        };
      }),
      ...
      commitMutation((state) => {
        ...
        return {
          ...
          project: {
            ...
            objects: updateObjectById(state.project.objects, id, (item) => ({
              ...item,
              // 姿势与动作互斥：摆造型自动关掉动作循环（保留速度/幅度等调参）
              characterRig: item.characterRig
                ? {
                    ...item.characterRig,
                    posePresetId: presetId,
                    controls: preset ? { ...preset.controls } : item.characterRig.controls,
                    motion: item.characterRig.motion
                      ? { ...item.characterRig.motion, kind: "none" }
                      : item.characterRig.motion,
                  }
                : item.characterRig,
            })),
          },
    ...
    setMotion: (id, kind) =>
      commitMutation((state) => ({
        ...
        project: {
          ...state.project,
          objects: updateObjectById(state.project.objects, id, (item) =>
            item.kind === "character" ? applyMotionSelectionToCharacter(item, kind) : item
          ),
        },
      })),
    setCrowdMotion: (crowdId, kind) =>
      commitMutation((state) => ({
        ...state,
        project: {
          ...state.project,
          objects: state.project.objects.map((item) =>
            item.kind === "character" && item.crowdId === crowdId ? applyMotionSelectionToCharacter(item, kind) : item
          ),
        },
      })),
    setMotionSpeed: (id, speed) =>
      commitMutation((state) => ({
        ...state,
        project: {
          ...state.project,
          objects: updateObjectById(state.project.objects, id, (item) =>
            item.kind === "character" ? applyMotionPatchToCharacter(item, { speed }) : item
          ),
        },
      })),
    setCrowdMotionSpeed: (crowdId, speed) =>
      commitMutation((state) => ({
        ...state,
        project: {
          ...state.project,
          objects: state.project.objects.map((item) =>
            item.kind === "character" && item.crowdId === crowdId ? applyMotionPatchToCharacter(item, { speed }) : item
          ),
        },
      })),
    setMotionAmplitude: (id, amplitude) =>
      commitMutation((state) => ({
        ...state,
        project: {
          ...state.project,
          objects: updateObjectById(state.project.objects, id, (item) =>
            item.kind === "character" ? applyMotionPatchToCharacter(item, { amplitude }) : item
          ),
        },
      })),
    setCrowdMotionAmplitude: (crowdId, amplitude) =>
      commitMutation((state) => ({
        ...state,
        project: {
          ...state.project,
          objects: state.project.objects.map((item) =>
            item.kind === "character" && item.crowdId === crowdId ? applyMotionPatchToCharacter(item, { amplitude }) : item
          ),
        },
      })),
    setMotionEnabled: (id, enabled) =>
      commitMutation((state) => ({
        ...state,
        project: {
          ...state.project,
          objects: updateObjectById(state.project.objects, id, (item) =>
            item.kind === "character" ? applyMotionPatchToCharacter(item, { enabled }) : item
          ),
        },
      })),
    setCrowdMotionEnabled: (crowdId, enabled) =>
      commitMutation((state) => ({
        ...state,
        project: {
          ...state.project,
          objects: state.project.objects.map((item) =>
            item.kind === "character" && item.crowdId === crowdId ? applyMotionPatchToCharacter(item, { enabled }) : item
          ),
        },
      })),
    ...
    clearAllKeyframes: () =>
      commitMutation((state) => ({
        ...
        project: {
          ...state.project,
          timeline: { ...state.project.timeline, tracks: [], cameraTracks: [] },
        },
      })),
    copySelectedObjects: () => {
