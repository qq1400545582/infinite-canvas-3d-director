...
it("deletes the object's timeline track along with the object", () => {
  useDirectorStore.setState(createInitialDirectorState());
  const store = useDirectorStore.getState();
  store.addKeyframe("char_default_a", 1, {
    position: [0, 0, 0],
    rotation: [0, 0, 0],
    scale: [1, 1, 1],
  });
  expect(useDirectorStore.getState().project.timeline.tracks).toHaveLength(1);

  useDirectorStore.getState().selectObject("char_default_a");
  useDirectorStore.getState().deleteSelectedObject();

  expect(useDirectorStore.getState().project.timeline.tracks).toHaveLength(0);
});

it("deletes the linked camera track when a camera object is removed", () => {
  useDirectorStore.setState(createInitialDirectorState());
  const cameraObject = useDirectorStore
    .getState()
    .project.objects.find((item) => item.kind === "camera" && item.linkedCameraId === "cam_1")!;
  useDirectorStore.getState().addCameraKeyframe("cam_1", 1, {
    position: [1, 1, 1],
    target: [0, 0, 0],
    fov: 50,
  });
  expect(useDirectorStore.getState().project.timeline.cameraTracks).toHaveLength(1);

  useDirectorStore.getState().selectObject(cameraObject.id);
  useDirectorStore.getState().deleteSelectedObject();

  expect(useDirectorStore.getState().project.timeline.cameraTracks).toHaveLength(0);
});
...
it("replaceProject migrates a legacy v1 project into a v2 project with a timeline", () => {
  const legacyV1 = {
    version: 1,
    scene: { scale: 1, position: [0, 0, 0] as [number, number, number], rotation: [0, 0, 0] as [number, number, number], backgroundColor: "#000000", panoramaYaw: 0, panoramaRadius: 60, showLabels: true, snapToGrid: false, showGround: true, groundOpacity: 0.4, groundHeight: 0 },
    assets: [],
    objects: [],
    cameras: [],
    activeCameraId: null,
...
it("clearAllKeyframes empties both object and camera tracks", () => {
  useDirectorStore.getState().replaceProject(createDefaultDirectorProject() as never);
  const objectId = useDirectorStore.getState().project.objects[0].id;
  const cameraId = useDirectorStore.getState().project.cameras[0].id;
  useDirectorStore.getState().addKeyframe(objectId, 1, {
    position: [0, 0, 0], rotation: [0, 0, 0], scale: [1, 1, 1],
  });
  useDirectorStore.getState().addCameraKeyframe(cameraId, 1, {
    position: [0, 0, 0], target: [0, 0, 0], fov: 50,
  });

  useDirectorStore.getState().clearAllKeyframes();

  const timeline = useDirectorStore.getState().project.timeline;
