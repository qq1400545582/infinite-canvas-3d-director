...
+ [新增] 3D 导演台节点插件（jlmlh-3d-director）：以同源 iframe 内嵌 jlmlh-3d-director 三维导演台，可在节点插件管理器随意安装/关闭；导演台全部功能逻辑保持不变（模型导入、机位管理、时间线动画、截图导出到画布），并按节点 `instanceId` 隔离场景、复用宿主明暗主题、把截图作为上游图片资源输出。
